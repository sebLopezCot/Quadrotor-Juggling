...
class HandController(LeafSystem):
    ...
    def __init__(self, hand, x_nom, num_fingers,
        ...
        for i in range(num_fingers):
            self._DeclareVectorOutputPort(
                ...
                functools.partial(self._DoCalcVectorOutput, i))
    ...
    def ComputeControlInput(self, x, t):
        ...
        qdd = prog.NewContinuousVariables(self.nq, "qdd")
        qd_hand = prog.NewContinuousVariables(self.nq, "qd_hand")
        q_hand = prog.NewContinuousVariables(self.nq, "q_hand")
        
        betas = []
        lambd = []
        cone_vecs = []

        # Calculate the approriate friction cone vectors
        for i in range(n_cf):
            normal = grasp_normals_world_now[0:2,i]
            rot_mat = np.array([[0.0, 1.0], [-1.0, 0.0]])
            tangent = rot_mat.dot(normal)
            c0 = normal - self.mu*tangent
            c1 = normal + self.mu*tangent
            cone_vecs.append([c0, c1])

        # Set constraints on contact forces
        for i in range(n_cf):
            lambd.append(prog.NewContinuousVariables(2, "lambda_%d" % i))
            betas.append(prog.NewContinuousVariables(2, "betas_%d" % i))
            prog.AddLinearConstraint(betas[i][0] >= 0.0)
            prog.AddLinearConstraint(betas[i][1] >= 0.0)
            prog.AddLinearConstraint(lambd[i][0] == betas[i][0]*cone_vecs[i][0][0] + betas[i][1]*cone_vecs[i][1][0])
            prog.AddLinearConstraint(lambd[i][1] == betas[i][0]*cone_vecs[i][0][1] + betas[i][1]*cone_vecs[i][1][1])

        # Set constraints on the dynamics of movement and contact
        summer = None 
        for i in range(0,n_cf):
            if summer is None:
                summer = J_contact[:,i,:].T.dot(lambd[i])
            else:
                summer += J_contact[:,i,:].T.dot(lambd[i])

        for i in range(self.nq):
            prog.AddConstraint(M[i].dot(qdd) + C[i] == B[i].dot(u) + summer[i])
        
        # Integrate the states forward
        qd_hand = v + dt*qdd
        q_hand = q + dt*qd_hand
        ...
        # Apply costs on the state error and state velocity
        prog.AddQuadraticCost(Kp*(qdes - q_hand).dot(qdes - q_hand))
        prog.AddQuadraticCost(Kd*qd_hand.dot(qd_hand))
    ...
    def _DoCalcVectorOutput(self, i, context, y_data):
...
def SimulateHand(duration=10., 
    ...
    contact_log._DeclarePeriodicPublish(0.0333, 0.0)
